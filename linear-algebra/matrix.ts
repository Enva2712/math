...
export default class Matrix {
	...
	static fromString(string: string): Matrix {
		...
		const cells = rows
			.flat()
			.map(cell => new Fraction(cell.trim() || 0))
		return new Matrix(cells, rows[0].length)
	}

	static tag(entries: TemplateStringsArray, ...values: unknown[]): Matrix {
		const string = entries.reduce((acc, str, i) => `${acc}${str}${i < values.length ? values[i] : ''}`, '')
	...
	toString(): string {
		...
		return rows
			.map(r => r
				.map(c => c.padStart(longestEntry + 1))
				.join(', ')
			)
			.join('\n')
	}
	...
	getIndex(row: number, column: number): number {
		if(this.width < column || this.cells.length/this.width < row || row < 1 || column < 1) throw new Error('Index out of range')
	...
	*columns(): Generator<Fraction[]> {
	...
	static add(m1: Matrix, m2: number | Fraction | Matrix): Matrix {
		...
		if(m2 instanceof Fraction) {
			return new Matrix(m1.cells.map(c => c.add(m2 as Fraction)), m1.width)
		}
		if(m1.width !== m2.width || m1.cells.length !== m2.cells.length) throw new Error('Matrices must be the same dimentions to be added');
	...
	static sub(m1: Matrix, m2: Matrix | Fraction | number): Matrix {
		if(typeof m2 === 'number') m2 = new Fraction(m2)
		if(m2 instanceof Fraction) return new Matrix(m1.cells.map(c => c.add(m2 as Fraction)), m1.width)
		if(m1.width !== m2.width || m1.cells.length !== m2.cells.length) throw new Error('Matrices must be the same dimentions to be subtracted');
	...
	static det(m: Matrix): Fraction {
		if(!m.isSquare()) throw new Error('A matrix must be square to compute its determinant');
	...
	add(m: number | Fraction | Matrix): Matrix { return Matrix.add(this, m); }
	sub(m: number | Fraction | Matrix): Matrix { return Matrix.sub(this, m); }
	mul(m: number | Fraction | Matrix): Matrix { return Matrix.mul(this, m); }
	det(): Fraction { return Matrix.det(this) }
}
